...
export const generateColorArray = (colorRandomAmount: number): Array<string> => {
	const colorsArray = [];
...
function interpolateColor(startColor: RGB, endColor: RGB, factor: number): RGB {
	return {
...
export const generateFadePallet = (firstColor: RGB, secondColor: RGB) => {
	const colorGradient: RGB[] = [];
	const res = 250
	for (let i = 0; i < res; i++) {
